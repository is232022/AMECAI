...
def estimate( 
        y, x, z=None, transform='', T=None, robust_se=False, N=None
    ) -> list:
    ...
    p_val = scipy.stats.t.sf(abs(t_values), df=N-1)
    
    names = ['b_hat', 'se', 'sigma2', 't_values', 'R2', 'cov', 'p_val']
    results = [b_hat, se, sigma2, t_values, R2, cov, p_val]
...
def print_table(
        labels: tuple,
        results: dict,
        headers=["", "Beta", "Se", "t-values", "p_val"],
        title="Results",
        _lambda:float=None,
        **kwargs
    ) -> None:
